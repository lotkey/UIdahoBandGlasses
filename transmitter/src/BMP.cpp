...
void BMP::load(std::filesystem::path const& img_path)
{
  ...
  infile.read(reinterpret_cast<char*>(&m_fileHeader), sizeof(m_fileHeader));
  if (m_fileHeader.file_type != 0x4D42) {
    throw std::runtime_error("Error! Unrecognized file format.");
  }
  infile.read(reinterpret_cast<char*>(&m_infoHeader), sizeof(m_infoHeader));
  ...
  if (32 == m_infoHeader.bit_count) {
    ...
    if (m_infoHeader.size >= (sizeof(InfoHeader) + sizeof(ColorHeader))) {
      infile.read(reinterpret_cast<char*>(&m_colorHeader),
                  sizeof(m_colorHeader));
  ...
  if (32 == m_infoHeader.bit_count) {
    m_infoHeader.size += sizeof(ColorHeader);
    m_fileHeader.offset_data += sizeof(ColorHeader);
  }
  ...
  if (0 == m_infoHeader.width % 4) {
    infile.read(reinterpret_cast<char*>(m_data.data()), m_data.size());
    m_fileHeader.file_size += m_data.size();
  } else {
    m_rowStride = m_infoHeader.width * m_infoHeader.bit_count / 8;
    std::uint32_t new_stride = makeStrideAligned(4);
    std::vector<std::uint8_t> padding_row(new_stride - m_rowStride);

    for (int y = 0; y < m_infoHeader.height; ++y) {
      infile.read(reinterpret_cast<char*>(m_data.data() + (m_rowStride * y)),
                  m_rowStride);
      infile.read(reinterpret_cast<char*>(padding_row.data()),
                  padding_row.size());
    }
    m_fileHeader.file_size +=
      m_data.size() + m_infoHeader.height * padding_row.size();
  }
}
...
BMP::BMP(Image const& img)
{
  ...
  std::uint32_t new_stride = makeStrideAligned(4);
  m_fileHeader.file_size = m_fileHeader.offset_data +
                           static_cast<std::uint32_t>(m_data.size()) +
...
void BMP::save(std::filesystem::path const& img_path) const
{
  ...
  if (32 == m_infoHeader.bit_count) {
    ...
    if (0 == m_infoHeader.width % 4) {
      ...
      for (int y = 0; y < m_infoHeader.height; ++y) {
        outfile.write(
          reinterpret_cast<char const*>(m_data.data() + m_rowStride * y),
          m_rowStride);
        outfile.write(reinterpret_cast<char const*>(padding_row.data()),
                      padding_row.size());
      }
    }
  } else {
...
void BMP::set(int row, int col, common::Color const& color)
{
  ...
  std::uint32_t channels = m_infoHeader.bit_count / 8;
...
{
  outfile.write(reinterpret_cast<char const*>(&m_fileHeader),
                sizeof(m_fileHeader));
  outfile.write(reinterpret_cast<char const*>(&m_infoHeader),
                sizeof(m_infoHeader));
  if (32 == m_infoHeader.bit_count) {
    outfile.write(reinterpret_cast<char const*>(&m_colorHeader),
                  sizeof(m_colorHeader));
  }
}
...
{
  outfile.write(reinterpret_cast<char const*>(m_data.data()), m_data.size());
}

// Add 1 to the m_rowStride until it is divisible with align_stride
std::uint32_t BMP::makeStrideAligned(std::uint32_t align_stride) const
{
  std::uint32_t new_stride = m_rowStride;
