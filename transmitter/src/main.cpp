 ...
 */
int sendFlash(ftdi_context* ftdi,
              transmitter::Image& image,
              int imageX,
              int imageY,
              common::Color const& newColor);
int main()
{
  // srand(time(NULL));
  ...
  if (setupFTDIConnection(ftdi) != 0) { return EXIT_FAILURE; }
  ...
  do {
    ...
    switch (letter) {
      ...
      case 'g':
        sendFlash(
          ftdi, imageToSend, imageX, imageY, transmitter::colors::Green);
        break;
      default: usleep(DAB);
    }
    nbytes =
      sendFlash(ftdi, imageToSend, imageX, imageY, transmitter::colors::Blank);
    // Draw a space over current character
  } while (letter != '.');

  if (closeFTDIConnection(ftdi) != 0) { return EXIT_FAILURE; }
...
int setupFTDIConnection(ftdi_context* ftdi)
{
  ...
  for (curdev = devlist; curdev != NULL; i++) {
    ...
    if ((errors = ftdi_usb_get_strings(
           ftdi, curdev->dev, manufacturer, 128, description, 128, NULL, 0)) <
        0) {
      fprintf(stderr,
  ...
  if (errors < 0) {
    fprintf(stderr,
            "unable to set line parameters: %d (%s).\n",
            errors,
            ftdi_get_error_string(ftdi));
    return errors;
  } else {
