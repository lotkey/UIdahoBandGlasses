...
int main()
{
    ...
    do {
        ...
        switch (letter) {
            case 'r':
                sendFlash(
                  ftdi, imageToSend, imageX, imageY, transmitter::colors::Red);
                break;

            case 'b':
                sendFlash(
                  ftdi, imageToSend, imageX, imageY, transmitter::colors::Blue);
                break;
            case 'g':
                sendFlash(ftdi,
                          imageToSend,
        ...
        nbytes = sendFlash(
          ftdi, imageToSend, imageX, imageY, transmitter::colors::Blank);
...
int setupFTDIConnection(ftdi_context* ftdi)
{
    ...
    for (curdev = devlist; curdev != NULL; i++) {
        ...
        if ((errors = ftdi_usb_get_strings(ftdi,
                                           curdev->dev,
                                           manufacturer,
                                           128,
                                           description,
