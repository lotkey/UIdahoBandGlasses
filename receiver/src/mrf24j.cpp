...
static int bytes_nodata =
    bytes_MHR + bytes_FCS; // no_data bytes in PHY payload,  header length + FCS

static int ignoreBytes = 0; // bytes to ignore, some modules behaviour.

static boolean bufPHY =
    false; // flag to buffer all bytes in PHY Payload, or not
 ...
 * it continue working. Only the most recent data is ever kept.
 */
    ...
    if (last_interrupt & MRF_I_RXIF) {
        ...
        if (bufPHY) {
            ...
            for (int i = 0; i < frame_length;
                 i++) { // from 0x301 to (0x301 + frame_length -1)
...
void Mrf24j::check_flags(void (*rx_handler)(void), void (*tx_handler)(void)) {
    // TODO - we could check whether the flags are > 1 here, indicating data was
    // lost?
    if (flag_got_rx) {
        flag_got_rx = 0;
        rx_handler();
...
int Mrf24j::rx_datalength(void) { return rx_info.frame_length - bytes_nodata; }
 ...
 */
void Mrf24j::set_bufferPHY(boolean bp) { bufPHY = bp; }

boolean Mrf24j::get_bufferPHY(void) { return bufPHY; }
